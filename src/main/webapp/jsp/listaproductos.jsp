    ...
    <div class="col-md-3">
        <form action="Peticion" method="post">
            <fieldset class="container-fluid">
                ...
                <div class="form-group">
                    <label for="oferta" id="oferta" class="checkbox-inline">
                    ...
                    <tbody>
                        <c:forEach var="producto" items="${productos}">
                            <tr>
                                ...
                                <td class="productos-item-precio text-right"><fmt:formatNumber type="currency" value="${producto.precioUnitario}"/></td>
                                <c:if test="${sessionScope.USUARIO.tipo == 'u'.charAt(0)}">
                                    <c:choose>
                                        <c:when test="${producto.stock > 0}">
                                            <td><button class="btn btn-default">A&ntilde;adir al Carrito</button></td>
                                        </c:when>
                                        <c:when test="${producto.stock == 0}">
                                            <td><h4 class="text-center">AGOTADO</h4></td>
                                        </c:when>
                                    </c:choose>
                                </c:if>
                            </tr>
